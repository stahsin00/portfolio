...
function Nav() {
    ...
    return (
        <>
            <button className='fixed top-5 left-5 bg-black hover:bg-teal-700 text-white p-2 rounded-2xl border-2 border-white border-solid md:hidden' onClick={() => setClosed(!closed)}><FaBars size={32} /></button>
            <nav className={`fixed top-20 md:top-0 space-y-3 p-5 md:space-y-0 md:space-x-5 ${closed ? 'hidden' : 'flex'} md:flex flex-col md:flex-row`}>
